...
def spend_confirm():
    ...
    payment_method = request.form.getlist('payMethod')
    if message_user == "":
    ...
    flash(f'Spende von "{user_name}"\nüber {pay_value[0]},00 € ist erfolgreich abgeschlossen\nVielen herzlichen Dank im Namen aller Tiere und dem Team!')
...
def save_payment_data(paymentdata):
    with open("logs/payment-log.log", "a") as f:
        f.write(json.dumps(paymentdata))
